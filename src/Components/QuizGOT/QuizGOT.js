

export const QuizGOT = {
    
    debutant: [
        {
            id: 0,
            quiz: `Comment s’appelle le loup de Jon Snow ?`,
            options: [
                'Croc-Blanc',
                'Snoopy',
                'Drogon',
                'Fantôme',
            ],
            answer: 'Fantôme',
            description: `Le loup de Jon Snow a pour nom Fantôme. Un patronyme lié à sa couleur, un blanc immaculé. Il fait partie des six louveteaux trouvés lors du premier épisode par Ned Stark, le seigneur de Winterfell, qui les répartit à chacun de ses enfants, y compris à Jon, qui est pourtant son fils illégitime. Fantôme est l’un des rares loups des Stark à avoir survécu tout au long de la série, suivant fidèlement Jon dans ses aventures.`,
            image: 'https://static.wikia.nocookie.net/gameofthrones/images/9/94/804_Ghost_Profile.png/revision/latest?cb=20190506030424'
        },
        {
            id: 1,
            quiz: `De quelle cité l’épouse de Robb Stark est-elle originaire ?`,
            options: [
                'Quarth',
                'Pentos',
                'Volantis',
                'Accalmie',
            ],
            answer: 'Volantis',
            description: `Talisa Stark est une jeune noble originaire de Volantis, une des neuf cités libres situées à l’est de Westeros, de l’autre côté du Détroit. On la découvre dans la deuxième saison, alors qu'elle soigne les blessés pour l’armée de Robb Stark, engagé dans sa guerre contre les Lannister. Leur rencontre, puis leur mariage, auront des conséquences terribles pour la famille Stark.`,
            image: 'https://pbs.twimg.com/media/EqbV9URXMAAxX0Z.jpg'
        },
        {
            id: 2,
            quiz: `Qu’offre Drogo à Daenerys lors de leur mariage ?`,
            options: [
                'Un cheval blanc',
                'Un dragon blanc',
                'Une couronne blanche',
                'Ure robe blanche',
            ],
            answer: 'Un cheval blanc',
            description: `Khal Drogo, seigneur du peuple guerrier des Dothrakis, offre à Daenerys un cheval blanc le jour de leurs noces. Dans la société dothrakie, le cheval tient une place centrale : "Dothraki" signifie "cavalier", et quand un seigneur dothraki n’est plus capable de monter à cheval, il est abandonné par les siens.`,
            image: 'https://static.wikia.nocookie.net/game-of-thrones-le-trone-de-fer/images/2/2a/Argent%C3%A9.jpg/revision/latest?cb=20170810124355&path-prefix=fr'
        },
        {
            id: 3,
            quiz: `D’où vient la balafre de Tyrion ?`,
            options: [
                `Il est tombé de son lit après une nuit de beuverie`,
                `Il a été blessé pendant la bataille de la Néra`,
                `Il a été blessé lors d'un séjour au-delà du Mur`,
                `Il a été frappé par Gregor Clegane, alias "La Montagne"`,
            ],
            answer: `Il a été blessé pendant la bataille de la Néra`,
            description: `Tyrion, le nain et le cadet des Lannister, a été grièvement blessé lors de la bataille de la Néra, survenue lorsque Stannis Baratheon a tenté de prendre Port-Réal. Après avoir dirigé la destruction de la flotte de Stannis, Tyrion se lance dans la mêlée et reçoit un grand coup d’épée au visage porté par Ser Mandon Moore, qui est pourtant un membre de la garde royale de son neveu Joffrey. Il en réchappe et se réveille plusieurs jours plus tard, défiguré.`,
            image: 'https://figurinepop.com/public/tyrion2_1.jpg'
        },
        {
            id: 4,
            quiz: `Un seul de ces Greyjoy n’existe pas, lequel ?`,
            options: [
                `Euron Greyjoy`,
                `Yara Greyjoy`,
                `Theon Greyjoy`,
                `Baelor Greyjoy`,
            ],
            answer: `Baelor Greyjoy`,
            description: `Il ne faut pas confondre Baelor et Balon. Baelor était un Targaryen, un ancien roi très pieux qui a donné son nom au septuaire de Port-Réal, centre névralgique de la "foi des sept". Balon Greyjoy est, lui, le seigneur des Iles de fer. Il est le père de Theon Greyjoy (qui fut otage chez les Stark) et le frère d’Euron Greyjoy. C’est d’ailleurs ce dernier qui le tue pour prendre sa place, avant de s'allier avec Cersei Lannister.`,
            image: 'https://static.wikia.nocookie.net/a-song-of-seven-kingdoms/images/f/f2/Baelor.jpg/revision/latest?cb=20181125013535'
        },
        {
            id: 5,
            quiz: `Par qui Robb Stark est-il tué lors des Noces pourpres ?`,
            options: [
                `Ramsay Bolton`,
                `Roose Bolton`,
                `Walder Frey`,
                `Walder Rivers`,
            ],
            answer: `Roose Bolton`,
            description: `Roose Bolton, autrefois fidèle allié de Ned Stark et de son fils Robb, s'est révélé être un traître de haute volée. Il était le compagnon du "roi du Nord" lors du nouveau mariage du vieux Walder Frey, mariage qui était en réalité un piège sanglant ourdi par les Lannister. Roose Bolton achève Robb Stark, dont le corps est déjà criblé de flèches, d’un coup de poignard dans le cœur.`,
            image: 'https://i.pinimg.com/originals/ce/16/e1/ce16e1db5363d04d979a68420c734d14.jpg'
        },
        {
            id: 6,
            quiz: `Comment s’appelle le compagnon de Bran, capable d’avoir des visions ?`,
            options: [
                `Joren`,
                `Joguen`,
                `Jojen`,
                `Johen`,
            ],
            answer: `Jojen`,
            description: `Jojen Reed est le fils d’Howland Reed, un allié de Ned Stark. Quand le jeune Bran et son frère Rickon fuient Winterfell dans la saison 3, ils rencontrent Jojen, qui voyage avec sa sœur Meera. Jojen, qui a des visions, révèle à Bran qu’il a lui aussi des pouvoirs.`,
            image: 'https://static.wikia.nocookie.net/game-of-thrones-le-trone-de-fer/images/1/10/The_Rains_of_Castamere_3x09_%2810%29.jpg/revision/latest/scale-to-width-down/250?cb=20160630133857&path-prefix=fr'
        },
        {
            id: 7,
            quiz: `Parmi ces personnages, lequel n’a jamais souhaité, à un moment de la série, être l'époux de Daenerys ?`,
            options: [
                `Hizdahr zo Loraq`,
                `Xaro Xhoan Daxos`,
                `Viserys Targaryen`,
                `Khal Drogo`,
            ],
            answer: `Viserys Targaryen`,
            description: `La Mère des dragons a souvent attiré les prétendants. Drogo l’a épousée dès le début de la série avant de mourir rapidement. A Qarth, le riche marchand Xaro Xhoan Daxos lui propose le mariage pour obtenir les moyens de reconquérir Westeros. A Meereen, le noble Hizdahr zo Loraq devient le fiancé officiel de Dany, pour sceller une alliance, mais il mourra avant la noce. En revanche, bien que les Targaryen n’aient rien contre les mariages incestueux, jamais Viserys, le grand frère de Daenerys, n’a songé à l’épouser.`,
            image: 'https://static.wikia.nocookie.net/gameofthrones/images/4/46/Viseryspromoheadshot.jpg/revision/latest?cb=20210722093457'
        },
        {
            id: 8,
            quiz: `Où est née Daenerys ?`,
            options: [
                `A Port-Réal`,
                `A Pentos`,
                `A Peyredragon`,
                `Sur un bateau`,
            ],
            answer: `A Peyredragon`,
            description: `L’un des surnoms de Daenerys Targaryen est "Née du typhon", car la Mère des dragons a vu le jour à Peyredragon, un jour de cyclone. Elle a ensuite fui vers l'est avec sa mère Rhaella et son frère Viserys.`,
            image: 'https://www.lagardedenuit.com/wall/wp-content/uploads/2017/07/20170720-GOT-Peyredragon-918x516.jpg'
        },
        {
            id: 9,
            quiz: `Quelle est la devise des Lannister ?`,
            options: [
                `Je rugis`,
                `Un Lannister paie toujours ses dettes`,
                `Dorés comme jamais`,
                `Lion d’or, griffes dehors`,
            ],
            answer: `Je rugis`,
            description: `L’expression "un Lannister paie toujours ses dettes" est très usitée dans la série, mais c’est un simple proverbe. La vraie devise des Lannister est bien "Je rugis !", eux dont le blason est un lion d'or rugissant sur champ écarlate.`,
            image: 'https://wir.skyrock.net/wir/v1/resize/?c=isi&im=%2F8371%2F93488371%2Fpics%2F3249296232_1_3_GmoHGVcH.jpg&w=400'
        }
    ],

    amateur: [
        {
            id: 0,
            quiz: `Comment s’appelle le poison qui tue Joffrey le jour de son mariage avec Margaery Tyrell ?`,
            options: [
                `Les frissons de Pentos`,
                `L’étrangleur`,
                `Les larmes de Lys`,
                `Le feu des dragons`,
            ],
            answer: `L’étrangleur`,
            description: `L’étrangleur est un poison qui se présente sous forme de cristaux. C’est lui qui tue Joffrey lors de ses noces, durant lesquelles il s'étouffe, avant de virer au bleu, le visage ensanglanté.`,
            image: 'https://static.wikia.nocookie.net/game-of-thrones-le-trone-de-fer/images/f/f3/Joffrey-agonisant.gif/revision/latest?cb=20170423113534&path-prefix=fr'
        },
        {
            id: 1,
            quiz: `Comment s’appelle le poison qui tue Joffrey le jour de son mariage avec Margaery Tyrell ?`,
            options: [
                `Sansa`,
                `Joffrey`,
                `Ned Stark`,
                `Ilyn Payne, le bourreau des Lannister`,
            ],
            answer: `Ned Stark`,
            description: `Dans la première saison, la louve d'Arya, baptisée Nymeria, mord sévèrement le prince Joffrey au cours d’une bagarre. La bête est condamnée à mort par le roi Robert, mais, Nymeria s'étant enfuie, c'est Lady, la louve de Sansa, qui est injustement tuée à sa place par Ned Stark. `,
            image: 'https://file1.telestar.fr/var/telestar/storage/images/3/1/9/8/3198725/lady-louve-sansa-stark.jpg?alias=original'
        },
        {
            id: 2,
            quiz: `A qui appartient la dague qui devait tuer Bran et qui tue le Roi de la nuit ?`,
            options: [
                `A Tyrion Lannister`,
                `A Arya Stark`,
                `A Joffrey Baratheon`,
                `A Littlefinger`,
            ],
            answer: `A Littlefinger`,
            description: `Cette arme est au centre de la guerre qui fait tout basculer dès la première saison. Un tueur armé de cette dague tente d'assassiner le jeune Bran Stark à Winterfell. On sait que l’arme a appartenu à Tyrion Lannister, mais surtout à Littlefinger, le maître des complots. C’est avec cette même lame qu’Arya tue le Roi de la nuit à l’issue de la bataille de Winterfell, dans la dernière saison.`,
            image: 'https://upload.wikimedia.org/wikipedia/en/d/d5/Aidan_Gillen_playing_Petyr_Baelish.jpg'
        },
        {
            id: 3,
            quiz: `De qui est cette Citation : 'Quand le soleil se lèvera à l'ouest et se couchera à l'est, quand les mers s'assécheront et quand les montagnes s'envoleront dans le vent comme des feuilles'.`,
            options: [
                `Tyrion Lannister`,
                `Petyr Baelish`,
                `Khal Drogo`,
                `Mirri Maz Duur`,
            ],
            answer: `Mirri Maz Duur`,
            description: `Lors d'un duel, Khal Drogo est blessé à la poitrine. Sa blessure s'infecte, le précipitant vers la mort. Daenerys prie la guérisseuse et magicienne Mirri Maz Duur de le guérir avant de lui demander quand Drogo reprendra conscience. En version courte, sa réponse correspond à un "jamais" qui traumatise profondément Daenerys.`,
            image: 'https://static.wikia.nocookie.net/game-of-thrones-le-trone-de-fer/images/f/f4/Mirri_Maz_Duur.png/revision/latest/top-crop/width/360/height/360?cb=20161004223318&path-prefix=fr'
        },
        {
            id: 4,
            quiz: `Qui est assis sur le Trône de fer au début de la série ?`,
            options: [
                `Robert Baratheon`,
                `Ned Stark`,
                `Cersei Lannister`,
                `Aerys II`,
            ],
            answer: `Robert Baratheon`,
            description: `La série démarre en l'an 298 après la conquête du premier roi Targaryen, Aegon le Conquérant. Robert Baratheon est alors roi depuis dix-sept ans. Il meurt la même année, dans l'épisode 7 de la saison 1, des suites d'un accident de chasse.`,
            image: 'https://static.wikia.nocookie.net/gameofthrones/images/4/43/Robert_crop.png/revision/latest?cb=20210223122212'
        },
        {
            id: 5,
            quiz: `Les cruels Bolton sont des bannerets d’une grande famille. Laquelle ?`,
            options: [
                `Les Lannister`,
                `Les Baratheon`,
                `Les Stark`,
                `Les Tully`,
            ],
            answer: `Les Stark`,
            description: `Roose Bolton est officiellement un banneret de la maison Stark. Il jure d'ailleurs allégeance à Robb Stark, roi du Nord, dans la première saison. Les Bolton trahiront pourtant les Stark en faveur des Lannister dans la saison 2. La devise "Nos lames sont acérées" aurait pu mettre la puce à l'oreille des seigneurs de Winterfell.`,
            image: 'https://static.wikia.nocookie.net/gameofthrones/images/8/8a/House-Stark-Main-Shield.PNG/revision/latest?cb=20170101103142'
        },
        {
            id: 6,
            quiz: `Comment se nomment les dragons de Daenerys ?`,
            options: [
                `Balerion, Xerxès et Viserys`,
                `Drogon, Rhaegal et Dracarys`,
                `Rhaegal, Viserion et Drogon`,
                `Drogon, Rhaegal et Dracula`,
            ],
            answer: `Rhaegal, Viserion et Drogon`,
            description: `Emblèmes de la maison Targaryen, les trois dragons que possède Daenerys sont considérés comme les derniers représentants de leur espèce, censée être disparue. La Mère des dragons parvient à faire éclore trois œufs fossilisés en se plongeant avec eux dans le bûcher funéraire de son mari, Khal Drogo.`,
            image: 'https://static.hitek.fr/img/actualite/2016/05/06/w_capture-d-e-cran-2016-05-06-a-11-57-47.png'
        },
        {
            id: 7,
            quiz: `Quelle langue est la langue natale de Daenerys ?`,
            options: [
                `Le dothraki`,
                `La langue commune de Westeros`,
                `Le haut valyrien`,
                `Le Nathien`,
            ],
            answer: `Le haut valyrien`,
            description: `Les Targaryen sont les descendants de l'empire valyrien, dont c'était la langue. Mélisandre et Talisa Stark le parlent également.`,
            image: 'https://static.hitek.fr/img/actualite/2016/05/06/w_capture-d-e-cran-2016-05-06-a-11-57-47.png'
        },
        {
            id: 8,
            quiz: `D’où vient Jorah Mormont ?`,
            options: [
                `L’île aux Ours`,
                `L’île des Vents`,
                `Dorne`,
                `L’île des Crabes`,
            ],
            answer: `L’île aux Ours`,
            description: `Seigneur du Nord exilé à Essos, il était destiné à prendre la tête de la maison Mormont. Mais, confronté à des dettes contractées par sa femme, il s'est adonné au trafic d'esclaves, un crime passible de mort à Westeros. Il a dû fuir vers l'est pour échapper à la sentence fatale de Ned Stark, la famille Mormont étant vassale des seigneurs de Winterfell.`,
            image: 'https://upload.wikimedia.org/wikipedia/en/d/d5/Jorah_Mormont-Iain_Glen.jpg'
        },
        {
            id: 9,
            quiz: `Par qui Daenerys est-elle trahie dès le début de la série ?`,
            options: [
                `Illyrio Mopatis`,
                `Ned Stark`,
                `Jorah Mormont`,
                `Son frère Viserys`,
            ],
            answer: `Jorah Mormont`,
            description: `S'il jure de se mettre au service de Viserys Targaryen, Ser Jorah murmure d'abord à l'oreille de Lord Varys, "le maître des chuchoteurs" pour le compte du roi Robert Baratheon, dans l'espoir de pouvoir mettre fin à son exil forcé.`,
            image: 'https://upload.wikimedia.org/wikipedia/en/d/d5/Jorah_Mormont-Iain_Glen.jpg'
        }
    ],

    expert: [
        {
            id: 0,
            quiz: `Quelle scène a provoqué une polémique lors de la diffusion de la saison 5 ?`,
            options: [
                `La nuit de noces de Sansa`,
                `La bataille des bâtards`,
                `La séance de torture de Theon Greyjoy`,
                `La mort de Bran, l’enfant handicapé des Stark`,
            ],
            answer: `La nuit de noces de Sansa`,
            description: `Lors de l'épisode 6 de la saison 5, Ramsay Bolton viole Sansa Stark lors de leur nuit de noces, sous les yeux de Theon Greyjoy. La scène, qui ne figure pas dans les livres de George R. R. Martin, a choqué beaucoup de fans de la série.`,
            image: 'https://static.hitek.fr/img/actualite/ill_m/1806355030/ramsayboltonsansa.jpg'
        },
        {
            id: 1,
            quiz: `Comment se nomme l’épée que Jaime Lannister donne à Brienne de Torth ?`,
            options: [
                `Féroce`,
                `Féale`,
                `Griffe d’or`,
                `Régicide`,
            ],
            answer: `Féale`,
            description: `Jaime Lannister donne cette épée à Brienne dans la saison 4, quand il lui demande de partir à la recherche de Sansa. Décorée de têtes de lions, elle a été offerte à Jaime par son père, Tywin Lannister.`,
            image: 'https://media.cdnws.com/_i/46446/8121/3685/22/w6019-2.jpeg'
        },
        {
            id: 2,
            quiz: `Combien de rois sont engagés dans la guerre qui suit la mort de Robert Baratheon ?`,
            options: [
                `3`,
                `4`,
                `5`,
                `6`,
            ],
            answer: `5`,
            description: `Joffrey Baratheon monte sur le trône à la mort de son père "officiel", Robert Baratheon. Mais, doutant de sa filiation, les deux frères de Robert, Stannis et Renly, se battent pour le Trône de fer. C'est le début de la Guerre des Cinq Rois. De leur côté, Balon Greyjoy et Robb Stark veulent, eux, se détacher du royaume des Sept Couronnes et être reconnus respectivement comme roi des Iles de fer et comme roi du Nord.`,
            image: 'https://static.wikia.nocookie.net/game-of-thrones-le-trone-de-fer/images/e/ed/Robert_signe_ses_derni%C3%A8res_volont%C3%A9s_1x07.jpg/revision/latest/scale-to-width-down/1200?cb=20161230055119&path-prefix=fr'
        },
        {
            id: 3,
            quiz: `Complétez la phrase de Syrio Forel : "Il n’y a qu’un seul dieu, et ce dieu est la mort. Et on ne dit qu’une seule chose à la mort…"`,
            options: [
                `Tu ne m’auras pas`,
                `Va te faire voir à Astapor`,
                `Pas aujourd’hui`,
                `Reviens demain`,
            ],
            answer: `Pas aujourd’hui`,
            description: `Epéiste de Braavos, Syrio Forel est le "maître à danser" d'Arya, son maître d'armes choisi par Eddark Stark. Il lui apprend à répondre ainsi à la mort, le seul dieu qu'elle doit craindre, selon lui.`,
            image: 'https://media.gqmagazine.fr/photos/5b9917fbe1cf6200114c2082/16:9/w_2560%2Cc_limit/game_of_thrones___syrio_forel_est_il_mort___343.png'
        },
        {
            id: 4,
            quiz: `Quel groupe de musique joue lors de la fête organisée à Port-Réal pour le mariage de Joffrey et Margaery ?`,
            options: [
                `Ed Sheeran`,
                `Sigur Ros`,
                `Of Monsters and Men`,
                `Mastodon`,
            ],
            answer: `Sigur Ros`,
            description: `Le trio islandais Sigur Ros interprète Les Pluies de Castamere dans la saison 4. Le chanteur britannique Ed Sheeran fait, lui, une apparition dans la saison 7. Le groupe de métal américain Mastodon s’est glissé dans la peau de Sauvageons dans la saison 5 puis de Marcheurs blancs dans la saison 7. Et les Islandais d’Of Monsters and Men ont joué dans la saison 6.`,
            image: 'https://www.cnet.com/a/img/resize/c230631c2107a57a5032efb02a12e0d69d872fc2/2014/04/28/20194d46-32c0-488b-bf19-306f245ddbdf/sigur-ros1.jpg?auto=webp&fit=crop&height=630&width=1200'
        },
        {
            id: 5,
            quiz: `Combien de descendants ont eu Ned et Catelyn Stark ?`,
            options: [
                `3`,
                `4`,
                `5`,
                `6`,
            ],
            answer: `5`,
            description: `Ned et Catelyn Stark ont eu cinq enfants : trois fils (Robb, Bran et Rickon) et deux filles (Sansa et Arya). Jon Snow a été un temps présenté comme le bâtard de Ned, mais il est en fait l’enfant caché de Lyanna Stark et de Rhaegar Targaryen.`,
            image: 'https://www.goldderby.com/wp-content/uploads/2016/04/game-of-thrones-ned-catelyn-stark.jpg?w=620&h=360&crop=1'
        },
        {
            id: 6,
            quiz: `Qui offre les œufs de dragon à Daenerys ?`,
            options: [
                `Illyrio Mopatis`,
                `Jorah Mormont`,
                `Khal Drogo`,
                `Un marchand dont on ne connaît pas le nom`,
            ],
            answer: `Illyrio Mopatis`,
            description: `Négociant et marchand d'épices, Illyrio Mopatis accueille Viserys et Daenerys Targaryen à Pentos et organise le mariage de cette dernière avec Khal Drogo. A cette occasion, il lui offre trois œufs de dragon, provenant de contrées lointaines, qu'il pense être de simples pierres.`,
            image: 'https://static.wikia.nocookie.net/gameofthrones/images/d/d8/Illyrio-Mopatis-HD.png/revision/latest?cb=20190427031447'
        },
        {
            id: 7,
            quiz: `Quel est le nom des continents où se déroule l’intrigue ?`,
            options: [
                `Northos et Southos`,
                `Westeros et Essos`,
                `Northos et Estos`,
                `Westos et Southos`,
            ],
            answer: `Westeros et Essos`,
            description: `Tout au long de la série, les événements relatés se déroulent sur les deux continents à la fois. La grande distance qui les sépare fait que certains personnages centraux de l'intrigue ne se rencontrent jamais. Jusqu'à la dernière saison, qui se déroule exclusivement à Westeros.`,
            image: 'https://static.wikia.nocookie.net/gameofthrones/images/3/39/Westeros_and_Essos.jpg/revision/latest?cb=20120402102432'
        },
        {
            id: 8,
            quiz: `Qui tranche la main de Jaime Lannister ?`,
            options: [
                `Locke`,
                `Le Limier`,
                `Catelyn Stark`,
                `Le "titilleur"`,
            ],
            answer: `Locke`,
            description: `Brienne de Torth est chargée par Catelyn Stark d'escorter Jaime Lannister jusqu'à Port-Réal pour l'échanger contre ses filles captives. Sur la route, ils se font capturer par des hommes de Roose Bolton, dont Locke, le meilleur chasseur du seigneur de Fort-Terreur. Il tranche la main droite de Jaimie, celle qui faisait de lui une des meilleures lames du royaume.`,
            image: 'https://static.wikia.nocookie.net/game-of-thrones-le-trone-de-fer/images/9/98/Locke.png/revision/latest?cb=20160916190833&path-prefix=fr'
        },
        {
            id: 9,
            quiz: `Pourquoi Sandor Clegane, "le Limier" et, Gregor Clegane, "la Montagne", ne peuvent-ils pas se voir en peinture ?`,
            options: [
                `Parce qu’ils aimaient la même femme`,
                `Parce que leur mère est morte en mettant le plus jeune au monde`,
                `Parce que Gregor a brûlé et défiguré Sandor enfant`,
                `Parce qu’ils ont été élevés par des loups`,
            ],
            answer: `Parce que Gregor a brûlé et défiguré Sandor enfant`,
            description: `La haine qui anime les frères Clegane remonte à leur enfance. Violent dès le plus jeune âge, Gregor ne supporte pas de voir Sandor jouer avec ses affaires, et lui plonge la tête dans le feu, lui brûlant le visage.`,
            image: 'https://static0.srcdn.com/wordpress/wp-content/uploads/2020/02/game-of-thrones-hound-versus-the-mountain-Edited.jpg'
        }
    ]
}